## Task
ID: 0.2
Goal (restate in your own words, one sentence): Fix CRLF churn by normalizing line endings and adding a .gitattributes file.

## Files changed
.gitattributes

## Allowlist compliance
Every file above appears in the task's ALLOWED FILES list: YES
If NO, explain — and expect rejection: <...>

## Gate output
Command: `bash -c 'git add --renormalize . && git diff --cached --shortstat | grep -q . || echo "already normalized"; git commit -m "Normalize line endings" 2>/dev/null; git status --porcelain --untracked-files=no | tee /tmp/g02.txt; test ! -s /tmp/g02.txt && npm run verify && echo "GATE 0.2 PASS"'`
```
already normalized

> 4444555-arcade@0.1.0 verify
...
✔ games are kept behind the platform SDK boundary (4.3716ms)
✔ manifest identifies a versioned game and SDK (0.6243ms)
✔ asset budgets are internally consistent (0.123ms)
✔ Gravity Courier is lazy-loaded and version-aligned (4.567ms)
✔ production flight preserves accepted visuals and exposes a complete game loop (2.1447ms)
✔ Pages fallback contains true unlit obstacle correction 0.15 (10.4404ms)
ℹ tests 6
ℹ suites 0
ℹ pass 6
ℹ fail 0
...
## Behaviour change
Did anything the player sees, hears, or feels change? NO
